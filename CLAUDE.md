...
- `4_20_make_interface_goat_analysis` — GOAT enrichment + intersection analyses of interface
  subgraphs vs. the gene lists. **COVID→PD only**: there is no `4_25`, because BEL nodes carry
  only `uniprot` annotations while GOAT needs `ncbigene` and the intersection needs
  `hgnc.symbol`, and `Activity`/`Complex`/`Composite` nodes have no name of their own — a second
  gene-set backend, not a small change.
...
`UPSTREAM_COLLECTION_NAME` / `DOWNSTREAM_COLLECTION_NAME` / `INTERFACE_COLLECTION_NAMES` name the
COVID→PD pairing (`4_10`, `4_20`); `AD_UPSTREAM_COLLECTION_NAME` /
`AD_DOWNSTREAM_COLLECTION_NAME` / `AD_INTERFACE_COLLECTION_NAMES` name the COVID→AD one (`4_15`),
whose downstream side is the AD BEL KG. Every name in an interface tuple must be **distinct**:
...
- Collection names (`"COVID_DM_CD_AF"`, `"PD_DM_CD_AF"`, `"AD_KG_BEL"`, …) are referenced by
  ...
  `UPSTREAM_COLLECTION_NAME` / `DOWNSTREAM_COLLECTION_NAME` / `INTERFACE_COLLECTION_NAMES` and
  `AD_*` for the COVID→AD pairing, plus `BEL_COLLECTION_NAMES` for the ones with no stored
  CellDesigner elements.
- `queries.get_annotations` / `get_subunits` / `get_ids_and_context` all `MATCH
  (node:ModelElement)`, and **BEL nodes carry `:BELModelElement`, not `:ModelElement`** — so they
  return nothing for a BEL node, silently. `get_nodes` matches any node and is fine.
